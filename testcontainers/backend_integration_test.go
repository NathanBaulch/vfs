...
func registerOS(t *testing.T) string {
	tmpDir, err := os.MkdirTemp("", "backend_integration_test")
	require.NoError(t, err)
	t.Cleanup(func() { _ = os.Remove(tmpDir) })
	return fmt.Sprintf("file://%s/", filepath.ToSlash(tmpDir))
}
			...
			s.Run(tc.path+tc.fileName, func() {
				...
				s.True(
					strings.HasSuffix(dstSpaces.URI(), path.Join(tc.path, tc.fileName)) ||
						strings.HasSuffix(dstSpaces.URI(), path.Join(url.PathEscape(tc.path), url.PathEscape(tc.fileName))),
				...
				hasSuffix := strings.HasSuffix(newSrcSpaces.URI(), path.Join(tc.path, tc.fileName)) ||
					strings.HasSuffix(newSrcSpaces.URI(), path.Join(url.PathEscape(tc.path), url.PathEscape(tc.fileName)))

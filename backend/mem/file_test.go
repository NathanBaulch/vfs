...
func (s *memFileTest) TestRARO() {
	...
	s.Require().NoError(s.testFile.Close(), "unexpected error closing file") // close the file so buffer contents are stored
...
func (s *memFileTest) TestSeek2() {
	...
	s.Require().Error(err, "expected seek to throw an error")

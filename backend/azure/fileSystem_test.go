...
func (s *FileSystemTestSuite) TestParsePath() {
	...
	s.Require().NoError(err, "Path is valid so we should not get an error")
	...
	u, _ = url.Parse(uri)
	volume, path, err = ParsePath(u.Path)
	s.Require().Error(err, "a container is required so we should get an error")
	...
	u, _ = url.Parse(uri)
	volume, path, err = ParsePath(u.Path)
	s.Require().NoError(err, "File Path is valid so we should not get an error")
	s.Equal("my_container", volume)
